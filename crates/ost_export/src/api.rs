...
pub fn export_as_game_music(
...
) -> Result<MusicExportResult, String> {
    ...
    fs::create_dir_all(&output_music_folder_path).map_err(|e| e.to_string())?;
    ...
    for music_file in music_files {
        let input_path_filename = music_file
            .file_name()
            .ok_or("No filename".to_string())?
            .to_string_lossy()
            .to_string(); // Ex: song.wav
        ...
        operations::wav_to_ogg(&temp_trimmed_wav_path, &output_ogg_path)
            .map_err(|e| e.to_string())?;
        fs::remove_file(&temp_trimmed_wav_path).map_err(|e| e.to_string())?;
